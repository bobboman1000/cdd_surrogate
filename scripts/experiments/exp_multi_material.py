...
def run(train_data, target, drop_cols, name, gbt_jobs=None, device=0, num_folds=10, batch_size=64, max_epochs=1500):
    ...
    do_split = target == "stress"
    input_size = 22 # TODO Move this to dataset, make dataset express its size
    ...
    def evaluate():
        ...
        for k in range(num_folds):
            ...
            models = [
                SeqToSeqLSTM(input_size, 30, 2, 1, 1).with_pre_estimator(get_gbt_estimate(datamodule, gbt_jobs)),
                EncodedHiddenStateLSTM(input_size, 30, 2, 1, 1),
                EncodedHiddenStateGRU(input_size, 30, 2, 1, 1),
                RepeatedInputLSTM(input_size, 30, 1, 1),
                RawHiddenStateLSTM(input_size, 1, 1)
            ]
            
            if datamodule.split_dataset:
                psd_estimator = get_rf_regressor(datamodule)
                split_only_models = [
                    ChunkingEncodedHiddenStateLSTM(input_size, 30, 2, 1, 1).with_estimator(psd_estimator),
                ]
                models += split_only_models

            print(f"------------------------------------ Starting with fold {k} ----------------------------------------")
            for model in models:
                test_loss = train_torch_model(model, datamodule, device, max_epochs)
...
def train_torch_model(model, datamodule, device, max_epochs=1500):
...
if __name__ == '__main__':
    ...
    run(data, "stress", ["strain", "time_ns", "dislocation"], "stress" + suffix, gbt_jobs, device, num_folds, batch_size, max_epochs)
    run(data, "strain", ["stress", "time_ns", "dislocation"], "strain" + suffix, gbt_jobs, device, num_folds, batch_size, max_epochs)
    run(data, "dislocation", ["time_ns", "stress", "strain"], "dislocation" + suffix, gbt_jobs, device, num_folds, batch_size, max_epochs)
